    ...
    <Card className="w-full max-w-md mx-auto relative">
      ...
      <CardContent className="text-center text-sm text-muted-foreground">
        {showAuthLinks ? (
          ...
          <p>The invitation may have been revoked, already used, or the link might be incorrect.</p>
        )}
      </CardContent>
      <CardFooter className="flex justify-center gap-2">
        {showAuthLinks ? (
          <>
            <Button asChild>
              <Link href={`/login?callbackUrl=/accept-invitation/${invitationId}`}>Sign In</Link>
            </Button>
